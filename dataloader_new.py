...
class GetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pair, label = self.get_pair(idx)
        annot[:, 4] = annot[:, 5]==label
...
def collater(data):
    ...
    padded_pairs = torch.zeros(batch_size, np.array([int(s.shape[0]) for s in pairs]).max(), np.array([int(s.shape[1]) for s in pairs]).max(), 3)
...
class Resizer(object):
    ...
    def __call__(self, sample, min_side=608, max_side=1024):
        ...
        return {'img': torch.from_numpy(new_image), 'annot': torch.from_numpy(annots), 'scale': scale, 'pair': torch.from_numpy(new_pair)}
