...
from roi_pooling import pyramid_roi_align#, Pooler
...
#     '''
#     def __init__(self, C2_size, C3_size, C4_size, C5_size):
#         super(PyramidFeatures, self).__init__()
        
#         # Bottom-up layers
#         self.conv6 = nn.MaxPool2d(kernel_size=1, stride=2)
...
#         self.latlayer4 = nn.Conv2d(C2_size, 256, kernel_size=1, stride=1, padding=0)
...
class ResNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.training:
            ...
            siamese_loss = F.binary_cross_entropy_with_logits(out, labels)
            ...
            boxes = transformed_anchors[:, anchors_nms_idx, :]
            pooled_feats = pyramid_roi_align([boxes]+features, 6, img_batch.shape[2:])
            # pooled_feats = self.pooler(features, boxes)
            similarity = self.siameseNetwork(pooled_feats, pairs)
